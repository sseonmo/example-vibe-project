...
1. **계획 없이 절대 구현하지 않는다.** 구현 전에 반드시 계획(Plan)을 먼저 세우고 사용자 승인을 받은 뒤 코드를 작성한다.
2. **모든 코드는 TDD로 작성한다.** 테스트를 먼저 작성하고 실패를 확인한 뒤(Red), 통과하는 구현을 작성하고(Green), 테스트로 검증한다.
3. **실제 실행으로 확인하기 전에 "동작한다"고 보고하지 않는다.** 테스트나 dev 서버로 직접 검증하지 않은 것은 추정으로만 말한다.
4. **요청받지 않은 파일은 수정하지 않는다.** 특히 아래 "의도적으로 혼재된 패턴"은 강의 자료이므로 지나가며 정리하지 말 것.
